...
const props = defineProps(["id", "path", "keys", "row"]);
...
<template>
  ...
  >
    <ElForm
      ...
      :inline="true"
      :rules="formRules"
      :model="form.queryForm"
      label-width="120px"
      label-suffix=":"
    >
      <ElFormItem label="路由等级">
        <el-select
          v-model="form.queryForm.menulev"
          :disabled="!!form.id"
          placeholder="Select"
      ...
      <ElFormItem label="选择路由" prop="menu">
        <ElSelect
          v-model="form.queryForm.menu"
          :disabled="!!form.id"
          placeholder="选择路由"
          @change="rowKey"
        >
          <el-option
            v-for="item in form.choiceMenuData"
            :key="item.id"
      ...
      <ElFormItem label="类型" prop="type">
        <el-radio-group v-model="form.queryForm.type">
          ...
          <el-radio-button value="insert" label="insert"> 写 </el-radio-button>
          <el-radio-button value="update" label="update"> 改 </el-radio-button>
          <el-radio-button value="delete" label="delete"> 删 </el-radio-button>
        </el-radio-group>
      </ElFormItem>
      <ElFormItem label="对应接口">
        <el-select
          v-model="form.queryForm.key"
          :disabled="!!form.id"
          placeholder="Select"
          @change="selectparentid"
        >
          <el-option
            v-for="item in form.keys"
            :key="item"
            :label="item"
            :value="item"
          />
